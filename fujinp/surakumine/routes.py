...
"""
surakumine - ルート定義（v1.0．すらくみ v2.3 からの分岐）

  すらくみ（slack_minutes）とはコード・画面とも同じ作りで，相手にする Slack
  ワークスペースだけが違う．違いは Config.SURAKUMINE_BOT_TOKEN /
  SURAKUMINE_BOT_NAME / SURAKUMINE_WORKSPACE_NAMES を読むこと，テーブルが
  surakumine_* であること，添付が fujinp/surakumine/data/files/ に入ること，
  URL が /surakumine 以下であることの4点．以下の版番号は分岐元のもの．
...
【v2.0 変更点：完全アーカイブ化】
  ...
  - ユーザー名・チャンネル名は DB（surakumine_users / _channels）に
...
try:
    ...
    _SLACK_SDK_OK = False
    logging.warning("surakumine: slack_sdk が未インストールです")
...
def _user_active_group_ids(cur, user_id) -> set:
    ...
    if _fn is not None:
        try:
            ...
            logging.warning("surakumine: user_groups.get_user_group_ids: %s", e)
    try:
        now = get_jst_now()
        cur.execute("""
        ...
        logging.warning("surakumine: user_group_memberships: %s", e)
        return set()
...
def _all_user_groups(cur) -> list:
    ...
    try:
        ...
        logging.warning("surakumine: user_groups: %s", e)
        return []


def _channel_access_group_ids(cur, channel_id) -> set:
    cur.execute("SELECT group_id FROM surakumine_access_groups "
...
def _check_channel_access(cur, channel_id, conn=None):   # conn は互換のため残置
    ...
    cur.execute("SELECT * FROM surakumine_channels WHERE channel_id=%s",
                (channel_id,))
    ch = cur.fetchone()
    if _is_admin():
        return ch
...
def _slack_client():
    token = getattr(Config, 'SURAKUMINE_BOT_TOKEN', None)
    if not token:
        raise RuntimeError('SURAKUMINE_BOT_TOKEN が config.py に設定されていません')
...
def _workspace_info() -> dict:
    ...
    names = getattr(Config, 'SURAKUMINE_WORKSPACE_NAMES', None)
    if isinstance(names, str):
        names = [names]
    bot_name = getattr(Config, 'SURAKUMINE_BOT_NAME', None)
    ...
    if not names or not bot_name:
        try:
            ...
            logging.warning("surakumine._workspace_info: %s", e)
            names = names or []
...
#
# v1.x はプロセス内キャッシュのみだったが，v2.0 では surakumine_users に
...
def _resolve_username(client, user_id: str, conn=None) -> str:
    ...
    if not _user_cache and conn is not None:
        try:
            ...
            logging.warning("surakumine: users cache load failed: %s", e)
    ...
    try:
        ...
        if conn is not None:
            try:
                ...
                logging.warning("surakumine: users upsert failed: %s", e)
        return name
    except Exception:
...
def _channel_name_resolver(conn):
    """<#C123> の解決関数（DB の surakumine_channels と messages から）"""
    names = {}
    try:
        cur = conn.cursor(dictionary=True)
...
def _fetch_and_save(channel_id: str, channel_name: str,
    ...
    try:
        now = get_jst_now()
        cur.execute("""
            INSERT INTO surakumine_sessions
        ...
        for msg in messages_raw:
            ...
            ts = msg.get('ts', '')
            cur.execute("""
                SELECT id FROM surakumine_messages
            ...
            cur.execute("""
                INSERT INTO surakumine_messages
        ...
        conn.rollback()
        logging.error("surakumine._fetch_and_save error: %s", e)
        if session_id:
            try:
                cur.execute("UPDATE surakumine_sessions SET status='error' "
...
def _register_files(cur, channel_id, msg, now):
    """メッセージ内の添付ファイルを surakumine_files に登録（pending）"""
    ts = msg.get('ts', '')
...
def _download_file(token, row) -> tuple:
    ...
    req = urllib.request.Request(url, headers={
        ...
        'User-Agent': 'FUJIN-P surakumine/2.0',
    })
    try:
...
def _sync_channel_info(conn, client, channel_id, channel_name):
    """surakumine_channels を更新（名前・種別・topic・purpose）"""
    now = get_jst_now()
    info = {}
    try:
        ...
        logging.warning("surakumine: conversations_info failed: %s", e)
    cur = conn.cursor()
    cur.execute("""
        INSERT INTO surakumine_channels
...
def _archive_step(sid: int) -> dict:
    ...
    client = _slack_client()
    token = getattr(Config, 'SURAKUMINE_BOT_TOKEN', '')
    conn = _db()
    cur = conn.cursor(dictionary=True)
    started = time.time()
    ...
    try:
        cur.execute("SELECT * FROM surakumine_sessions WHERE id=%s", (sid,))
        srow = cur.fetchone()
        if not srow:
        ...
        try:
            ...
            if state['phase'] == 'threads':
                ...
                if state['thread_idx'] >= len(state['threads']):
                    cur.execute("""
                        SELECT COUNT(*) AS c FROM surakumine_files
        ...
        if done:
            cur.execute("""
                UPDATE surakumine_channels SET last_archived_at=%s
                WHERE channel_id=%s
        ...
        logging.error("surakumine._archive_step error: %s", e)
        try:
            conn.rollback()
            cur.execute("UPDATE surakumine_sessions SET status='error' "
                        "WHERE id=%s", (sid,))
...
def _file_url(f, absolute=False):
    return url_for('surakumine.serve_file', fid=f['id'], _external=absolute)


def _files_md(files, absolute=False, url_fn=None, inline_images=False) -> str:
    """添付ファイルの Markdown 断片（リンク．inline_images なら画像は埋め込み）

    url_fn: 添付行 → URL を返す関数．None なら /file/<id>（サイト内配信）．
    """
    lines = []
    for f in files or []:
        ...
        if f.get('status') == 'done' and f.get('local_path'):
            url = url_fn(f) if url_fn else _file_url(f, absolute)
            mt = f.get('mimetype') or ''
            # ★v2.3 サイト内では画像も埋め込み（![]()）にせずリンクにする．添付は
            #   チャンネルの閲覧権で配信され，画像表示は公開領域に置いたものだけ，
            #   という規則に揃える．配布版（inline_images）では埋め込む
            if inline_images and mt.startswith('image/') and 'svg' not in mt:
...
def _message_md(row, files, resolve_user, resolve_channel, absolute=False,
                with_meta=True, level=3, url_fn=None,
                inline_images=False) -> str:
    """1メッセージの Markdown（見出し＋本文＋添付＋リアクション）"""
    body = mrkdwn_to_md(row.get('text') or '', resolve_user, resolve_channel)
    fm = _files_md(files, absolute, url_fn, inline_images)
    if fm:
        body = (body + '\n\n' + fm) if body else fm
...
def _md_to_html(md: str) -> str:
    ...
    if _site_md is not None:
        try:
            ...
            logging.warning("surakumine: process_markdown failed: %s", e)
    if _md_lib is not None:
        try:
            return _md_lib.markdown(md, extensions=['fenced_code', 'tables',
                                                    'nl2br'])
        except Exception as e:
            logging.warning("surakumine: markdown failed: %s", e)
    import html as _h
...
def _channel_row(cur, channel_id):
    ...
    if ch:
        return ch
    cur.execute("""
        SELECT channel_name FROM surakumine_messages
...
def _build_channel_md(conn, channel_id, absolute=True, url_fn=None,
                      inline_images=False, files=None) -> tuple:
    """チャンネル全体の Markdown．戻り値 (channel_name, md)

    url_fn を渡すと添付リンクの URL をその関数で作る（配布用zip）．
    files を渡すと添付一覧を DB から引かずそれを使う（配布用zipの添付なし版）．
    """
    cur = conn.cursor(dictionary=True)
...
@surakumine_bp.route('/return_to_fujin')
def return_to_fujin():
...
def index():
    ...
    if _is_admin():
        return render_template('surakumine/index.html')
    conn = _db()
    try:
        channels = _list_archives(conn, include_hidden=False)
    finally:
        conn.close()
    return render_template('surakumine/browse.html', channels=channels)


@surakumine_bp.route('/api/workspace', methods=['GET'])
@admin_required
def api_workspace():
...
@surakumine_bp.route('/api/channels', methods=['GET'])
@admin_required
def api_channels():
    """Slackチャンネル一覧を返す"""
    if not _SLACK_SDK_OK:
...
@surakumine_bp.route('/api/probe/<channel_id>', methods=['GET'])
@admin_required
...
def _slack_error_json(e):
    err = e.response.get('error', str(e))
    logging.error("surakumine SlackApiError: %s", err)
...
@surakumine_bp.route('/api/fetch', methods=['POST'])
@admin_required
def api_fetch():
...
@surakumine_bp.route('/api/archive/start', methods=['POST'])
@admin_required
...
@surakumine_bp.route('/api/archive/step/<int:sid>', methods=['POST'])
@admin_required
...
def _list_archives(conn, include_hidden: bool) -> list:
    ...
    rows = cur.fetchall()
    # 許可グループ（チャンネル別）
    cur.execute("SELECT channel_id, group_id FROM surakumine_access_groups")
    allowed = {}
    ...
    for r in rows:
        ...
        r['url'] = url_for('surakumine.channel_view',
                           channel_id=r['channel_id'])
        r['export_url'] = url_for('surakumine.export_md',
                                  channel_id=r['channel_id'])
        out.append(r)
    return out


@surakumine_bp.route('/api/archives', methods=['GET'])
@login_required
def api_archives():
...
@surakumine_bp.route('/api/archives/<channel_id>/access', methods=['GET'])
@admin_required
def api_get_access(channel_id):
    ...
    try:
        conn = _db()
        cur = conn.cursor(dictionary=True)
        cur.execute("SELECT visibility FROM surakumine_channels WHERE channel_id=%s",
                    (channel_id,))
        r = cur.fetchone()
...
@surakumine_bp.route('/api/archives/<channel_id>/access', methods=['POST'])
@admin_required
def api_set_access(channel_id):
    ...
    try:
        conn = _db()
        cur = conn.cursor(dictionary=True)
        cur.execute("SELECT channel_id FROM surakumine_channels WHERE channel_id=%s",
                    (channel_id,))
        if cur.fetchone():
            cur.execute("""
                UPDATE surakumine_channels SET visibility=%s, updated_at=%s
                WHERE channel_id=%s
        ...
        cur.execute("DELETE FROM surakumine_access_groups WHERE channel_id=%s",
                    (channel_id,))
        for gid in group_ids:
            cur.execute("""
                INSERT INTO surakumine_access_groups (channel_id, group_id)
                VALUES (%s, %s)
            """, (channel_id, gid))
        conn.commit()
...
@surakumine_bp.route('/channel/<channel_id>')
@login_required
def channel_view(channel_id):
    ...
    return render_template('surakumine/channel.html', ch=ch,
                           is_admin=_is_admin())


@surakumine_bp.route('/api/catalog/<channel_id>', methods=['GET'])
@login_required
def api_catalog(channel_id):
    ...
    try:
        ...
        ru = lambda uid: _user_cache.get(uid, uid)
        rc = _channel_name_resolver(conn)
        items = _catalog_items(parents, replies, files, ru, rc)
...
@surakumine_bp.route('/api/thread/<channel_id>/<ts>', methods=['GET'])
@login_required
def api_thread(channel_id, ts):
    ...
    try:
        ...
        cur.execute("""
            SELECT id, file_id, slack_ts, name, title, mimetype, size,
                   local_path, status
            FROM surakumine_files
            WHERE channel_id=%s AND (slack_ts=%s OR slack_ts IN (
                SELECT slack_ts FROM surakumine_messages
...
def _catalog_items(parents, replies, files, ru, rc) -> list:
    """目録の項目（api_catalog と配布版で共用）"""
    items = []
    for p in parents:
...
def _zip_member_name(f) -> str:
    """配布zip内での添付ファイル名（files/ 直下．file_id で一意）"""
    return os.path.basename(f['local_path'])


def _build_standalone(conn, channel_id, with_files=True) -> dict:
    """配布用zipの材料．戻り値 {'name','html','md','files':[(zip内パス, 実パス)]}"""
    ...
    if with_files:
        for lst in files.values():
            for f in lst:
                if f.get('status') == 'done' and f.get('local_path'):
                    real = os.path.normpath(os.path.join(FILES_DIR, f['local_path']))
                    if real.startswith(os.path.normpath(FILES_DIR)) and os.path.isfile(real):
                        members.append(('files/' + _zip_member_name(f), real))
                    else:
                        f['status'] = 'error'      # 実体なし→リンクにしない
        url_fn = lambda f: 'files/' + quote(_zip_member_name(f))
    else:
        for lst in files.values():
            for f in lst:
                f['status'] = 'omitted'
    ...
    for p in parents:
        out = []
        for r in [p] + replies.get(p['slack_ts'], []):
            md = _message_md(r, files.get(r['slack_ts']), ru, rc,
                             with_meta=False, url_fn=url_fn,
                             inline_images=with_files)
            out.append({'ts': r['slack_ts'],
                        'is_reply': r['slack_ts'] != p['slack_ts'],
                        'sender': r.get('sender_name') or '（不明）',
                        'posted_at': fmt_datetime(r.get('posted_at')),
                        'edited': bool(r.get('edited_at')),
                        'md': md, 'html': _md_to_html(md)})
        threads[p['slack_ts']] = out

    data = {'channel': {'name': ch['name'], 'is_private': bool(ch.get('is_private')),
                        'topic': ch.get('topic') or '', 'purpose': ch.get('purpose') or ''},
            'generated_at': fmt_datetime(get_jst_now()),
            'total': len(parents),
            'reply_total': sum(len(v) for v in replies.values()),
            'senders': sorted({p.get('sender_name') or '（不明）' for p in parents}),
            'with_files': with_files,
            'items': items, 'threads': threads}
    # <script> 内に埋め込むので '</' を無害化する
    data_json = json.dumps(data, ensure_ascii=False).replace('</', '<\\/')
    html = render_template('surakumine/standalone.html', ch=ch,
                           data_json=data_json,
                           generated_at=data['generated_at'])
    _, md = _build_channel_md(conn, channel_id, absolute=False, url_fn=url_fn,
                              inline_images=with_files, files=files)
    return {'name': ch['name'], 'html': html, 'md': md, 'files': members}


@surakumine_bp.route('/export/<channel_id>.md', methods=['GET'])
@login_required
...
@surakumine_bp.route('/export/<channel_id>.zip', methods=['GET'])
@admin_required
def export_zip(channel_id):
    """配布用zip（index.html＋channel.md＋添付）．?files=0 で添付を含めない

    配布したzipはすらくみねの閲覧権限の外に出る．admin 専用．
    """
    import tempfile
    import zipfile
    from flask import after_this_request
    with_files = request.args.get('files', '1') != '0'
    conn = _db()
    try:
        pack = _build_standalone(conn, channel_id, with_files)
    finally:
        conn.close()
    stamp = get_jst_now().strftime('%Y%m%d_%H%M')
    readme = (f'#{pack["name"]} の Slack アーカイブ（配布版）\n'
              f'出力：{stamp}　FUJIN-P すらくみね\n\n'
              'index.html をブラウザで開いてください（ダブルクリックで開けます）．\n'
              'channel.md は同じ内容の Markdown です．\n'
              + ('files/ に添付ファイルが入っています．\n' if with_files else
                 'この版には添付ファイルを含めていません．\n')
              + '\nこの記録は参加者向けに配布したものです．'
              '受け取った範囲の外へは転送しないでください．\n')
    tmp = tempfile.NamedTemporaryFile(suffix='.zip', delete=False)
    tmp.close()
    with zipfile.ZipFile(tmp.name, 'w', zipfile.ZIP_DEFLATED) as z:
        z.writestr('index.html', pack['html'])
        z.writestr('channel.md', pack['md'])
        z.writestr('README.txt', readme)
        for member, real in pack['files']:
            z.write(real, member)
    ...
    return send_file(tmp.name, mimetype='application/zip', as_attachment=True,
                     download_name=f'slack_{pack["name"]}_{stamp}.zip')


@surakumine_bp.route('/file/<int:fid>', methods=['GET'])
@login_required
def serve_file(fid):
    ...
    try:
        cur = conn.cursor(dictionary=True)
        cur.execute("SELECT * FROM surakumine_files WHERE id=%s", (fid,))
        f = cur.fetchone()
        if f:
...
@surakumine_bp.route('/api/sessions', methods=['GET'])
@admin_required
def api_sessions():
    """取得セッション一覧（新しい順）"""
    try:
...
@surakumine_bp.route('/api/messages/<int:sid>', methods=['GET'])
@admin_required
...
@surakumine_bp.route('/api/delete/<int:sid>', methods=['POST'])
@admin_required
def api_delete(sid):
    """
    ...
    try:
        conn = _db()
        cur  = conn.cursor()
        cur.execute("DELETE FROM surakumine_sessions WHERE id = %s", (sid,))
        conn.commit()
        return jsonify({'success': True})
    except Exception as e:
...
# 進行ブリーフ（★v1.1）
#
#   ウェビナー中の進行調整のための画面．押すたびに Slack の新着だけを
#   取り込み，直近 N 分の発言一覧と，AI（ふっこ経由の Claude）による
#   進行メモを admin の画面に出す．Slack への投稿は行わない．
#
#   /brief                画面
#   POST /api/brief/sync  新着取得＋発言一覧を返す（速い）
#   POST /api/brief/note  同じ範囲を AI に渡して進行メモを返す（数十秒）
# ══════════════════════════════════════════════════════════════

BRIEF_WINDOWS           = (15, 30, 60, 180, 0)  # 画面で選べる対象時間（分．0=全部）
BRIEF_HISTORY_PAGES     = 2                  # 1回の取得で読む履歴ページ数
BRIEF_MAX_THREADS       = 20                 # 返信を見に行くスレッド数の上限
BRIEF_THREAD_LOOKBACK_H = 24                 # 何時間前までの親スレッドを見るか
BRIEF_MAX_LINES         = 300                # 一覧・AI に渡す最大件数
BRIEF_MAX_FILE_DL       = 20                 # ★v1.3 1回の取得で実体を保存する添付数の上限
BRIEF_PRESEN_TYPES      = {'pptx', 'ppt', 'pdf', 'key', 'odp', 'gslides'}  # ★v1.3 プレゼン資料とみなす種別

BRIEF_INSTRUCTION = """あなたは，オンライン講義（ウェビナー）を進行している講師の補佐です．
受講者が Slack に書き込んだ直近のメッセージを渡します．講師は講義をしながらこれを画面で読みます．
要約して短くすることが目的ではありません．誰がどんな発言をしたかを取りこぼさず示し，進行の判断を助けることが目的です．

次の形式で，日本語の Markdown で書いてください．

## いま拾うべきもの
その場で触れた方がよい発言を，重要な順に．各行に「発言者名：内容（時刻）」を書き，なぜ拾うべきかを一言添える．

## あとに回せるもの
急がない質問・感想・雑談を，同じ形式で．

## 論点のまとまり
似た質問が複数あれば，論点ごとに束ねて，誰と誰が同じことを聞いているかを示す．

## 気づいたこと
未回答のまま流れている質問，操作や接続のトラブルを訴えている人，議論が噛み合っていない箇所など，進行上の注意点があれば書く．無ければ「特になし」．

守ること．
- 発言者の名前は渡されたとおりに書く．推測で補わない．
- 発言を落とさない．どの区分にも入らないものは「あとに回せるもの」に入れる．
- 講師への助言は簡潔に．評価や励ましは書かない．
- 末尾に「受講者の資料」の一覧が付いていることがある．これは参考情報で，一覧そのものは書き直さない（システムが別に付ける）．資料に触れた発言や，資料が未提出・差し替えに見える点があれば「気づいたこと」に書く．
"""


def _brief_channels(cur) -> list:
    """メッセージが1件でもあるチャンネルの一覧（新しい順）"""
    cur.execute("""
        SELECT m.channel_id,
               COALESCE(c.name, MAX(m.channel_name)) AS name,
               MAX(m.posted_at) AS last_at,
               COUNT(*) AS total
        FROM surakumine_messages m
        LEFT JOIN surakumine_channels c ON c.channel_id = m.channel_id
        ...
        ORDER BY last_at DESC
    """)
    return [{'channel_id': r['channel_id'], 'name': r['name'],
             'last_at': fmt_datetime(r['last_at']), 'total': r['total']}
            for r in cur.fetchall()]


def _brief_session_id(cur, conn, channel_id, channel_name, user_id) -> int:
    """保存に使うセッション．既存があれば流用し，取得履歴を汚さない"""
    cur.execute("SELECT id FROM surakumine_sessions WHERE channel_id=%s "
    ...
    cur.execute("""
        INSERT INTO surakumine_sessions
            (user_id, channel_id, channel_name, fetched_at, status, mode)
        VALUES (%s, %s, %s, %s, 'done', 'brief')
    """, (user_id, channel_id, channel_name or channel_id, get_jst_now()))
    conn.commit()
    return cur.lastrowid


def _brief_sync(channel_id, channel_name, user_id, minutes) -> dict:
    """直近 minutes 分の新着（本文・スレッド返信・リアクション）を取り込む"""
    client = _slack_client()
    conn = _db()
    cur = conn.cursor(dictionary=True)
    counts = {'fetched': 0, 'saved': 0, 'updated': 0, 'replies': 0}
    try:
        sid = _brief_session_id(cur, conn, channel_id, channel_name, user_id)
        now = get_jst_now()
        # minutes=0 は「全部」．Slack 側の期間指定を外す
        oldest = ('%.6f' % (time.time() - minutes * 60)) if minutes > 0 else None
        threads = []

        # ── 履歴（新着のみ）
        cursor = None
        for _ in range(BRIEF_HISTORY_PAGES):
            kw = dict(channel=channel_id, limit=HISTORY_PAGE_LIMIT)
            if oldest:
                kw['oldest'] = oldest
            if cursor:
            ...
            for msg in msgs:
                if msg.get('subtype') in SKIP_SUBTYPES:
                    continue
                r = _upsert_message(cur, sid, channel_id,
                                    channel_name or channel_id, msg,
                                    client, conn, now)
                if r == 'saved':
                    counts['saved'] += 1
                elif r == 'updated':
        ...
        # ── 少し前の親にも新しい返信が付くので，直近の親を見に行く
        cur.execute("""
            SELECT slack_ts FROM surakumine_messages
            WHERE channel_id=%s AND reply_count>0 AND posted_at>=%s
            ORDER BY posted_at DESC LIMIT %s
        """, (channel_id,
              get_jst_now() - datetime.timedelta(hours=BRIEF_THREAD_LOOKBACK_H),
              BRIEF_MAX_THREADS))
        for r in cur.fetchall():
            if r['slack_ts'] not in threads:
                threads.append(r['slack_ts'])

        for parent_ts in threads[:BRIEF_MAX_THREADS]:
            kw = dict(channel=channel_id, ts=parent_ts,
        ...
        # ── ★v1.3 添付の実体を保存する（資料リンクを進行メモで使えるように）
        token = getattr(Config, 'SURAKUMINE_BOT_TOKEN', '')
        cur.execute("""
            SELECT * FROM surakumine_files
            WHERE channel_id=%s AND status='pending'
            ORDER BY id LIMIT %s
        """, (channel_id, BRIEF_MAX_FILE_DL))
        for frow in cur.fetchall():
            try:
                status, rel, err = _download_file(token, frow)
            except Exception as e:
                status, rel, err = 'error', None, str(e)
            cur.execute("""
                UPDATE surakumine_files
                SET status=%s, local_path=%s, error=%s, downloaded_at=%s
                WHERE id=%s
            """, (status, rel, err,
                  get_jst_now() if status == 'done' else None, frow['id']))
            conn.commit()
            if status == 'done':
...
def _brief_rows(conn, channel_id, minutes) -> list:
    """直近 minutes 分（0=全部）の発言をスレッド構造で返す．本文は Markdown に復元する

    親メッセージを古い順に並べ，その直後に返信を（スレッド内の順に）ぶら下げる．
    返信の親が対象時間より前にあるときは，親を「文脈用」の行（is_stub）として
    返信の前に置く．件数が BRIEF_MAX_LINES を超えるときは新しい方を残す．
    """
    cur = conn.cursor(dictionary=True)
    if minutes > 0:
        since = get_jst_now() - datetime.timedelta(minutes=minutes)
        cur.execute("""
            SELECT * FROM surakumine_messages
            WHERE channel_id=%s AND posted_at>=%s
            ORDER BY posted_at DESC, id DESC LIMIT %s
        """, (channel_id, since, BRIEF_MAX_LINES))
    else:
        cur.execute("""
            SELECT * FROM surakumine_messages
            WHERE channel_id=%s
            ORDER BY posted_at DESC, id DESC LIMIT %s
        """, (channel_id, BRIEF_MAX_LINES))
    rows = cur.fetchall()
    rows.reverse()

    def parent_of(r):
        tts = r.get('thread_ts')
        return tts if (tts and tts != r['slack_ts']) else None

    in_view = {r['slack_ts']: r for r in rows}
    # 対象時間の外にある親を文脈用に引く
    missing = sorted({parent_of(r) for r in rows
                      if parent_of(r) and parent_of(r) not in in_view})
    stubs = {}
    if missing:
        ph = ','.join(['%s'] * len(missing))
        cur.execute(f"""
            SELECT * FROM surakumine_messages
            WHERE channel_id=%s AND slack_ts IN ({ph})
        """, (channel_id, *missing))
        stubs = {r['slack_ts']: r for r in cur.fetchall()}
    cur.close()
    ...
    # 親ごとに束ねる．並び順の基準時刻＝親が範囲内なら親の投稿時刻，
    # 親が範囲外（文脈用）または未取得なら最初の返信の投稿時刻
    groups = {}            # key_ts -> {'parent': row|None, 'stub': bool, 'replies': []}
    order  = []            # (sort_ts, key_ts)
    for r in rows:
        pts = parent_of(r)
        if pts is None:
            g = groups.setdefault(r['slack_ts'],
                                  {'parent': None, 'stub': False, 'replies': []})
            g['parent'] = r
            order.append((float(r['slack_ts']), r['slack_ts']))
        else:
            if pts not in groups:
                g = {'parent': stubs.get(pts), 'stub': pts in stubs, 'replies': []}
                groups[pts] = g
                if pts not in in_view:
                    order.append((float(r['slack_ts']), pts))
            groups[pts]['replies'].append(r)
    seen, keys = set(), []
    for _, k in sorted(order):
        if k not in seen:
            seen.add(k); keys.append(k)

    def entry(r, is_reply=False, is_stub=False, parent=None, parent_ts=None):
        label = ''
        if is_reply:
            label = (f"{parent.get('sender_name') or '（不明）'} "
                     f"{fmt_datetime(parent.get('posted_at'))}") if parent \
                    else '（親メッセージ未取得）'
        return {
            'ts':          r['slack_ts'],
            'time':        fmt_datetime(r.get('posted_at')),
            'sender':      r.get('sender_name') or '（不明）',
            'is_reply':    is_reply,
            'is_stub':     is_stub,
            'parent_ts':   parent_ts,
            'parent_label': label,
            'edited':      bool(r.get('edited_at')),
            'text':        mrkdwn_to_md(r.get('text') or '', ru, rc),
            'reactions':   _reactions_text(r.get('reactions_json')),
        }
    ...
    for k in keys:
        ...
        p = g['parent']
        if p is not None:
            out.append(entry(p, is_stub=g['stub']))
        for r in sorted(g['replies'], key=lambda x: float(x['slack_ts'])):
            out.append(entry(r, is_reply=True, parent=p, parent_ts=k))
    return out


def _brief_materials(conn, channel_id) -> list:
    """★v1.3 受講者ごとの資料（添付）を名寄せし，最新版へのリンクを付ける

    対象はチャンネルに保存された全期間の添付（対象時間で絞らない）．
    名寄せの鍵は sender_id（無ければ sender_name）．並びは各人の資料の
    初出（最初に添付を投稿した時刻）が早い順．各人につき，最新の添付と，
    それがプレゼン資料（BRIEF_PRESEN_TYPES）でないときは最新のプレゼン資料も返す．
    """
    cur = conn.cursor(dictionary=True)
    cur.execute("""
        SELECT f.id, f.file_id, f.name, f.title, f.filetype, f.mimetype,
               f.size, f.status, f.local_path, f.slack_ts,
               m.sender_id, m.sender_name, m.posted_at
        FROM surakumine_files f
        LEFT JOIN surakumine_messages m
               ON m.channel_id=f.channel_id AND m.slack_ts=f.slack_ts
        WHERE f.channel_id=%s
        ORDER BY m.posted_at, f.id
    """, (channel_id,))
    rows = cur.fetchall()
    cur.close()
    _load_users_from_db(conn)
    ...
    def is_presen(f):
        ...
        if ft in BRIEF_PRESEN_TYPES:
            return True
        name = (f.get('name') or '').lower()
        return any(name.endswith('.' + t) for t in BRIEF_PRESEN_TYPES if t != 'gslides')

    def pack(f):
        name = f.get('name') or f.get('title') or f.get('file_id')
        ok = f.get('status') == 'done' and bool(f.get('local_path'))
        note = {'expired': '期限切れ・取得不可', 'error': '取得失敗',
                'pending': '未取得'}.get(f.get('status'), '')
        return {'id': f['id'], 'name': name,
                'url': _file_url(f) if ok else None,
                'time': fmt_datetime(f.get('posted_at')),
                'size': format_size(f.get('size')),
                'presen': is_presen(f), 'note': '' if ok else note}

    people = {}
    order = []
    for f in rows:
        if not f.get('posted_at'):
            continue
        key = f.get('sender_id') or f.get('sender_name') or '?'
        p = people.get(key)
        if p is None:
            name = f.get('sender_name') or _user_cache.get(key, key)
            p = {'sender': name, 'first_at': f['posted_at'],
                 'first': fmt_datetime(f['posted_at']),
                 'count': 0, 'latest': None, 'latest_presen': None}
            people[key] = p
            order.append(key)
        p['count'] += 1
        p['latest'] = pack(f)          # posted_at 昇順なので最後が最新
        if is_presen(f):
            p['latest_presen'] = pack(f)
    out = []
    for key in sorted(order, key=lambda k: people[k]['first_at']):
        p = people[key]
        if p['latest_presen'] and p['latest'] and \
                p['latest_presen']['id'] == p['latest']['id']:
            p['latest_presen'] = None   # 最新がそのままプレゼン資料
        del p['first_at']
        out.append(p)
    return out


def _brief_materials_md(mats) -> str:
    """★v1.3 名寄せ結果の Markdown（進行メモの末尾に付ける）"""
    if not mats:
        return ''
    def link(f):
        s = f'（{f["size"]}）' if f['size'] else ''
        if f['url']:
            return f'[{f["name"]}]({f["url"]}){s}'
        return f'{f["name"]}{s} ※{f["note"]}'
    lines = ['## 受講者の資料（名寄せ・初出順）',
             f'{len(mats)} 名．各人の最新版へのリンク．',
             '']
    for i, p in enumerate(mats, 1):
        head = f'{i}. **{p["sender"]}**（初出 {p["first"]}・{p["count"]} 件）：'
        body = f'最新 {link(p["latest"])}（{p["latest"]["time"]}）'
        if p['latest_presen']:
            body += (f'／最新のプレゼン資料 {link(p["latest_presen"])}'
                     f'（{p["latest_presen"]["time"]}）')
        lines.append(head + body)
    return '\n'.join(lines)


def _brief_text(rows, channel_name, minutes, materials_md='') -> str:
    """AI に渡す平文．一覧そのもの（スレッド構造つき）"""
    span = f'直近 {minutes} 分' if minutes > 0 else '保存されている全期間'
    n = sum(1 for r in rows if not r.get('is_stub'))
    lines = [f'# チャンネル #{channel_name} の{span}の発言',
             f'（現在 {fmt_datetime(get_jst_now())}．{n} 件．'
             f'「↳」で始まる行はその直前の親メッセージへのスレッド返信）', '']
    for r in rows:
        if r.get('is_reply'):
            head = (f'↳ [{r["time"]}] {r["sender"]}'
                    + ('（親メッセージ未取得の返信）' if not r.get('parent_ts') or
                       r['parent_label'].startswith('（')
                       else f'（{r["parent_label"]} への返信）'))
            ind = '　　'
        else:
            head = f'[{r["time"]}] {r["sender"]}'
            if r.get('is_stub'):
                head += '（対象時間より前の投稿．返信の親として表示）'
            ind = ''
        if r['edited']:
            head += '（編集済み）'
        lines.append(ind + head)
        body = r['text'] or '（本文なし）'
        lines.extend(ind + l for l in body.split('\n'))
        if r['reactions']:
            lines.append(ind + f'リアクション：{r["reactions"]}')
        lines.append('')
    if materials_md:
        lines += ['---', '', materials_md, '']
    return '\n'.join(lines)
...
def _brief_ai(text):
    """ふっこ（fukko）経由で Claude に投げる．戻り値 (本文, エラー)"""
    try:
        from fujinp.fukko.routes import call_claude
    except Exception as e:
        logging.warning("surakumine._brief_ai: fukko import failed: %s", e)
        return None, ('ふっこ（fukko）が読み込めません．'
                      'AI なしの一覧だけをお使いください')
    try:
        reply, usage, err = call_claude(
            [{'role': 'user',
              'content': BRIEF_INSTRUCTION + '\n\n---\n\n' + text}])
    except Exception as e:
        logging.error("surakumine._brief_ai: %s", e)
        return None, 'AI 呼び出しでエラーが起きました: %s' % e
    if err:
        return None, err
    return reply, None


def _brief_minutes(data) -> int:
    """対象時間（分）．0 は「全部」なので or による既定値は使えない"""
    v = (data or {}).get('minutes')
    if v is None or v == '':
        return 30
    try:
        return max(0, int(v))
...
@surakumine_bp.route('/brief')
@admin_required
def brief_page():
    conn = _db()
    cur = conn.cursor(dictionary=True)
    try:
        channels = _brief_channels(cur)
    finally:
        cur.close()
        conn.close()
    return render_template('surakumine/brief.html',
                           channels=channels, windows=BRIEF_WINDOWS)


@surakumine_bp.route('/api/brief/sync', methods=['POST'])
@admin_required
def api_brief_sync():
    """新着を取り込み，直近 N 分の一覧を返す．Body: {channel_id, minutes, skip_fetch}"""
    if not _SLACK_SDK_OK:
        return jsonify({'success': False,
                        'error': 'slack_sdk が未インストールです'}), 500
    data       = request.json or {}
    channel_id = (data.get('channel_id') or '').strip()
    minutes    = _brief_minutes(data)
    skip_fetch = bool(data.get('skip_fetch'))
    if not channel_id:
        return jsonify({'success': False, 'error': 'チャンネル未指定です'}), 400
    conn = _db()
    try:
        cur = conn.cursor(dictionary=True)
        ch = _channel_row(cur, channel_id)
        ...
        if not skip_fetch:
            try:
                counts = _brief_sync(channel_id, ch['name'],
                                     session.get('user_id'), minutes)
            except RateLimited as e:
                return jsonify({'success': False,
                                'error': 'Slack の制限中です．%d 秒ほど待って'
                                         'もう一度押してください' % e.retry_after}), 429
            except SlackApiError as e:
                return _slack_error_json(e)
        rows = _brief_rows(conn, channel_id, minutes)
        mats = _brief_materials(conn, channel_id)          # ★v1.3
        return jsonify({'success': True, 'channel_name': ch['name'],
                        'minutes': minutes, 'now': fmt_datetime(get_jst_now()),
                        'counts': counts, 'rows': rows,
                        'materials': mats,
                        'materials_html': _md_to_html(_brief_materials_md(mats))
                                          if mats else '',
                        'count': sum(1 for r in rows if not r.get('is_stub')),
                        'truncated': sum(1 for r in rows
                                         if not r.get('is_stub')) >= BRIEF_MAX_LINES})
    except Exception as e:
        logging.error("api_brief_sync error: %s", e)
        return jsonify({'success': False, 'error': str(e)}), 500
    finally:
        conn.close()


@surakumine_bp.route('/api/brief/note', methods=['POST'])
@admin_required
def api_brief_note():
    """同じ範囲を AI に渡して進行メモを返す．Body: {channel_id, minutes}"""
    data       = request.json or {}
    channel_id = (data.get('channel_id') or '').strip()
    minutes    = _brief_minutes(data)
    if not channel_id:
    ...
    try:
        ...
        if not rows:
            return jsonify({'success': False,
                            'error': 'この時間内の発言がありません'}), 400
        mats_md = _brief_materials_md(_brief_materials(conn, channel_id))  # ★v1.3
        note, err = _brief_ai(_brief_text(rows, ch['name'], minutes, mats_md))
        if err:
            return jsonify({'success': False, 'error': err}), 500
        if mats_md:
            note = note.rstrip() + '\n\n' + mats_md
        return jsonify({'success': True, 'note': note,
                        'note_html': _md_to_html(note),
                        'count': sum(1 for r in rows if not r.get('is_stub')),
                        'now': fmt_datetime(get_jst_now())})
    except Exception as e:
        logging.error("api_brief_note error: %s", e)
        return jsonify({'success': False, 'error': str(e)}), 500
    finally:
        conn.close()